...
func controllerJobWriteValidations(message string) []admissionregistrationv1.Validation {
	validations := controllerObjectValidations(message,
		...
		`request.operation != "CREATE" || !has(dyn(object).status) || dyn(object.status).size() == 0`,
...
func controllerPlanContractExpression() string {
	...
	legacy := `variables.activeRelease == variables.previousRelease && dyn(object).spec.contractVersion == 2 && !has(dyn(object.spec).controllerImage) && !has(dyn(object.spec).controllerRevision) && !has(dyn(object.spec).controllerStateVersion)`
