...
func (c stagingSecretContract) secretShape(path string, requireLiveIdentity bool) string {
	...
	return fmt.Sprintf(
		`%[1]s != null && has(%[1]s.apiVersion) && %[1]s.apiVersion == "v1" && has(%[1]s.kind) && %[1]s.kind == "Secret" && has(%[1]s.metadata) && has(%[1]s.metadata.name) && %[1]s.metadata.name == %[2]q && has(%[1]s.metadata.namespace) && %[1]s.metadata.namespace == %[3]q && (!has(%[1]s.metadata.generateName) || %[1]s.metadata.generateName == "")%[4]s && has(%[1]s.metadata.labels) && %[1]s.metadata.labels == {%[5]q: %[6]q, "app.kubernetes.io/managed-by": "Helm"} && has(%[1]s.metadata.annotations) && %[1]s.metadata.annotations == {"meta.helm.sh/release-name": %[7]q, "meta.helm.sh/release-namespace": %[3]q} && (!has(%[1]s.metadata.ownerReferences) || %[1]s.metadata.ownerReferences.size() == 0) && (!has(%[1]s.metadata.finalizers) || %[1]s.metadata.finalizers.size() == 0) && !has(%[1]s.metadata.deletionTimestamp) && !has(%[1]s.metadata.deletionGracePeriodSeconds) && has(%[1]s.type) && %[1]s.type == "Opaque" && !has(%[1]s.immutable) && (!has(%[1]s.stringData) || %[1]s.stringData.size() == 0)`,
		path,
		c.stagingSecretName,
		c.namespace,
		liveIdentity,
...
func (g *StagingSecretGuard) verifyPolicy(actual, expected *admissionregistrationv1.ValidatingAdmissionPolicy) error {
	...
	if actual.APIVersion != expected.APIVersion || actual.Kind != expected.Kind {
		return errors.New("staging Secret guard policy has an unexpected API identity")
	}
...
func (g *StagingSecretGuard) verifyBinding(actual, expected *admissionregistrationv1.ValidatingAdmissionPolicyBinding) error {
	...
	if actual.APIVersion != expected.APIVersion || actual.Kind != expected.Kind {
		return errors.New("staging Secret guard binding has an unexpected API identity")
	}
