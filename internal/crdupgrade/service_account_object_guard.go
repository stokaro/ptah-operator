...
func (g *ServiceAccountObjectGuard) ExpectedPolicy() (*admissionregistrationv1.ValidatingAdmissionPolicy, error) {
	...
	objectShape := fmt.Sprintf(
		`object != null && object.apiVersion == "v1" && object.kind == "ServiceAccount" && object.metadata.namespace == %q && object.metadata.name == variables.name && (!has(object.metadata.generateName) || object.metadata.generateName == "") && (!has(object.metadata.ownerReferences) || object.metadata.ownerReferences.size() == 0) && (variables.isExternalController || !has(object.metadata.finalizers) || object.metadata.finalizers.size() == 0) && (!has(object.metadata.deletionTimestamp)) && (%s) && (%s) && (variables.isExternalController || ((variables.isManagedController || variables.isCertificate) ? (has(dyn(object).automountServiceAccountToken) && dyn(object).automountServiceAccountToken) : (has(dyn(object).automountServiceAccountToken) && !dyn(object).automountServiceAccountToken))) && ((variables.isManagedController || variables.isExternalController || variables.isCertificate) ? (!has(dyn(object).secrets) || dyn(object).secrets.all(secret, secret.name != "" && (!has(secret.namespace) || secret.namespace in ["", %q]) && (!has(secret.apiVersion) || secret.apiVersion in ["", "v1"]) && (!has(secret.kind) || secret.kind in ["", "Secret"]) && (!has(secret.uid) || secret.uid == "") && (!has(secret.resourceVersion) || secret.resourceVersion == "") && (!has(secret.fieldPath) || secret.fieldPath == ""))) : (!has(dyn(object).secrets) || dyn(object).secrets.size() == 0)) && ((variables.isManagedController || variables.isExternalController) ? (!has(dyn(object).imagePullSecrets) || dyn(object).imagePullSecrets.all(secret, secret.name != "")) : (!has(dyn(object).imagePullSecrets) || dyn(object).imagePullSecrets.size() == 0))`,
