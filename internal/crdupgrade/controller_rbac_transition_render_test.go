...
func TestControllerRBACCutoverHookRenderHasExactBoundedAuthority(t *testing.T) {
	...
	assertTransitionRenderRule(t, role, "rbac.authorization.k8s.io", "clusterroles", []string{controllerName}, []string{"get", "bind"})
	assertTransitionRenderRule(t, role, "rbac.authorization.k8s.io", "roles", []string{controllerName, controllerName + "-runtime-admission", controllerName + "-runtime-discovery"}, []string{"get", "bind"})
