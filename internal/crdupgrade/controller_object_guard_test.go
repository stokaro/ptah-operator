...
func TestControllerObjectGuardCELContracts(t *testing.T) {
	...
	for _, marker := range []string{
		`dyn(dyn(object).spec).scheduling`,
		`dyn(dyn(object).spec.template.spec).evictionResponders`,
		`dyn(dyn(object).spec.template.spec).workloadRef`,
		`dyn(dyn(oldObject).spec).scheduling`,
		`dyn(dyn(oldObject).spec.template.spec).evictionResponders`,
		`dyn(dyn(oldObject).spec.template.spec).workloadRef`,
	...
	for _, marker := range []string{
		...
		`has(dyn(dyn(object).spec).controllerRevision)`,
