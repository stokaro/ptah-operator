...
func (g *RolloutGuard) hookPodValidationExpressions(identityJob, preflightJob, reconcileJob, quiesceJob, teardownJob, teardownServiceAccount string) []string {
	...
	return []string{
		...
		fmt.Sprintf(`(%s == %q && %s) || (%s == %q && %s) || (%s == %q && %s) || (%s == %q && %s) || (%s == %q && %s)`, jobLabel, identityJob, g.hookArgsValidationExpression(container, "identity-probe"), jobLabel, preflightJob, g.hookArgsValidationExpression(container, "preflight"), jobLabel, reconcileJob, g.hookArgsValidationExpression(container, "reconcile"), jobLabel, quiesceJob, g.hookArgsValidationExpression(container, "teardown-quiesce"), jobLabel, teardownJob, g.hookArgsValidationExpression(container, "teardown")),
