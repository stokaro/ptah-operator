...
func (g *ParentWorkloadGuard) hookJobContractExpressions(identityJob, preflightJob, reconcileJob, quiesceJob, teardownJob, teardownServiceAccount string) []string {
	...
	return []string{
		...
		fmt.Sprintf(`(variables.isIdentity && %s) || (variables.isPreflight && %s) || (variables.isQuiesce && %s) || (variables.isTeardown && %s) || (!variables.isIdentity && !variables.isPreflight && !variables.isQuiesce && !variables.isTeardown && request.name == %q && %s)`, g.rollout.hookArgsValidationExpression(container, "identity-probe"), g.rollout.hookArgsValidationExpression(container, "preflight"), g.rollout.hookArgsValidationExpression(container, "teardown-quiesce"), g.rollout.hookArgsValidationExpression(container, "teardown"), reconcileJob, g.rollout.hookArgsValidationExpression(container, "reconcile")),
