...
// Keeping the resource does not keep its Job.
//
// Foreground cascading deletion removes an owner's dependents before the
// owner, and the Apply Job carries this resource as an owner with
// blockOwnerDeletion, so `kubectl delete --cascade=foreground` hands the
// executor to the garbage collector while the deletion wait is still polling
// it. The finalizer cannot help: it holds the owner, and the dependent goes
// first. Nothing in envtest or a fake client runs the garbage collector, so
// what is asserted here is the thing that makes it inapplicable -- the Job no
// longer names this resource as an owner.
func TestDeletingAMigrationDetachesTheApplyJobItIsWaitingOn(t *testing.T) {
	t.Parallel()
	...
	job.Status.Active = 1
	runningExecutorPod(pod)
	if len(job.OwnerReferences) == 0 || job.OwnerReferences[0].UID != migration.UID {
		t.Fatalf("the fixture Job is not owned by the migration: %#v", job.OwnerReferences)
	}
	reconciler, api := fakeMigrationReconciler(t, staticLogs{}, migration, plan, verificationPolicyConfigMap(), job, pod)
	holdMigrationApplyLease(t, reconciler, api, migration)
	if err := api.Delete(ctx, migration); err != nil {
	...
	detached := &batchv1.Job{}
	if err := api.Get(ctx, client.ObjectKey{Namespace: job.Namespace, Name: job.Name}, detached); err != nil {
		t.Fatal(err)
	}
	for _, owner := range detached.OwnerReferences {
		if owner.UID == migration.UID {
			t.Fatalf("the Apply Job still names the resource being deleted as an owner: %#v", detached.OwnerReferences)
		}
	}
	// The claim is still here, which is what the detachment is protecting.
	waiting := readMigration(t, api, migration)
	if waiting.Status.ActiveOperation == nil || waiting.Status.ActiveOperation.ID != operation.ID {
		t.Fatalf("the claim went while its Pod was still running: %#v", waiting.Status.ActiveOperation)
	}
