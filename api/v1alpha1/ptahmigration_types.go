...
type PtahMigrationStatus struct {
	...
	// release is written here in the same status patch that clears the claim
	// holding the Lease, and removed only once a release has succeeded, so a
	// manager that stops in between hands the realm back on its next pass
	// instead of leaving every other claimant to wait out the full lease.
	//
	// It is never written for a run that may still be executing. A Lease under
	// a live executor is the one thing the Lease exists to prevent, and such a
	// Lease is left to expire rather than recorded as owed.
