...
| `spec.artifact.transport` | `object` | This resource requires: artifact.transport.caFrom must name a required ConfigMap key. Transport is how the registry is reached: plain HTTP, a custom CA, a client certificate. |
| `spec.artifact.transport.caFrom` | `object` | This resource requires: artifact.transport.caFrom must name a required ConfigMap key. CAFrom selects a custom CA bundle. When registryAuthFrom is present, that same Secret must contain caSHA256 with the exact lowercase SHA-256 digest of the selected bytes. |
| `spec.artifact.transport.caFrom.key` | `string`, required | This resource requires: artifact.transport.caFrom must name a required ConfigMap key. The key to select. |
| `spec.artifact.transport.caFrom.name` | `string`, default `` | This resource requires: artifact.transport.caFrom must name a required ConfigMap key. Name of the referent. This field is effectively required, but due to backwards compatibility is allowed to be empty. Instances of this type with an empty value here are almost certainly wrong. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names |
| `spec.artifact.transport.caFrom.optional` | `boolean` | This resource requires: artifact.transport.caFrom must name a required ConfigMap key. Specify whether the ConfigMap or its key must be defined |
...
| `spec.artifact.verificationPolicyFrom.optional` | `boolean` | This resource requires: artifact.verificationPolicyFrom must name a required ConfigMap key. Specify whether the ConfigMap or its key must be defined |
| `spec.execution` | `object`, default `{}` | This resource requires: execution.connectTimeout must not exceed execution.activeDeadlineSeconds; policy.lockTimeout must not exceed execution.activeDeadlineSeconds. Execution shapes the Job a run happens in: its deadlines, its resources and the scheduling it inherits. |
| `spec.execution.activeDeadlineSeconds` | `integer`, default `900` | This resource requires: execution.connectTimeout must not exceed execution.activeDeadlineSeconds; policy.lockTimeout must not exceed execution.activeDeadlineSeconds. ActiveDeadlineSeconds is how long one operation Job may run before Kubernetes ends it. An apply that hits this leaves an uncertain outcome, which returns to observation rather than to a replay. |
...
| `spec.execution.connectTimeout` | `string`, default `10s` | This resource requires: execution.connectTimeout must not exceed execution.activeDeadlineSeconds. ConnectTimeout bounds opening the database connection, so an unreachable database fails in seconds rather than holding the Job to its deadline. |
...
| `spec.policy` | `object`, default `{}` | This resource requires: policy.lockTimeout must not exceed execution.activeDeadlineSeconds. Policy decides what may run without a person: the apply mode, the approval requirement, and the locks a run takes. |
