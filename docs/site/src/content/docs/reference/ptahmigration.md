...
| --- | --- | --- |
| `spec.artifact` | `object`, required | This resource requires: artifact.verificationPolicyFrom must name a required ConfigMap key; artifact.transport.caFrom must name a required ConfigMap key. Artifact is the OCI migration directory this history is matched against. It reuses the schema path's credential-isolated source contract: fetching an artifact never hands registry credentials to the process that runs SQL. |
...
| `spec.artifact.transport` | `object` | This resource requires: where `artifact.transport` is set, artifact.transport.caFrom must name a required ConfigMap key. Transport is how the registry is reached: plain HTTP, a custom CA, a client certificate. |
| `spec.artifact.transport.caFrom` | `object` | This resource requires: where `artifact.transport` and `artifact.transport.caFrom` are set, artifact.transport.caFrom must name a required ConfigMap key. CAFrom selects a custom CA bundle. When registryAuthFrom is present, that same Secret must contain caSHA256 with the exact lowercase SHA-256 digest of the selected bytes. |
| `spec.artifact.transport.caFrom.key` | `string`, required | This resource requires: where `artifact.transport` and `artifact.transport.caFrom` are set, artifact.transport.caFrom must name a required ConfigMap key. The key to select. |
| `spec.artifact.transport.caFrom.name` | `string`, default `` | This resource requires: where `artifact.transport` and `artifact.transport.caFrom` are set, artifact.transport.caFrom must name a required ConfigMap key. Name of the referent. This field is effectively required, but due to backwards compatibility is allowed to be empty. Instances of this type with an empty value here are almost certainly wrong. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names |
| `spec.artifact.transport.caFrom.optional` | `boolean` | This resource requires: where `artifact.transport`, `artifact.transport.caFrom` and `artifact.transport.caFrom.optional` are set, artifact.transport.caFrom must name a required ConfigMap key. Specify whether the ConfigMap or its key must be defined |
...
| `spec.artifact.verificationPolicyFrom` | `object`, required | This resource requires: artifact.verificationPolicyFrom must name a required ConfigMap key. VerificationPolicyFrom names the immutable ConfigMap holding the policy the artifact must satisfy. Editing it retires the plans computed under the previous version rather than letting them apply. |
| `spec.artifact.verificationPolicyFrom.key` | `string`, required | This resource requires: artifact.verificationPolicyFrom must name a required ConfigMap key. The key to select. |
| `spec.artifact.verificationPolicyFrom.name` | `string`, default `` | This resource requires: artifact.verificationPolicyFrom must name a required ConfigMap key. Name of the referent. This field is effectively required, but due to backwards compatibility is allowed to be empty. Instances of this type with an empty value here are almost certainly wrong. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names |
| `spec.artifact.verificationPolicyFrom.optional` | `boolean` | This resource requires: where `artifact.verificationPolicyFrom.optional` is set, artifact.verificationPolicyFrom must name a required ConfigMap key. Specify whether the ConfigMap or its key must be defined |
| `spec.execution` | `object`, default `{}` | This resource requires: where `execution` is set, execution.connectTimeout must not exceed execution.activeDeadlineSeconds; where `execution` is set, policy.lockTimeout must not exceed execution.activeDeadlineSeconds. Execution shapes the Job a run happens in: its deadlines, its resources and the scheduling it inherits. |
| `spec.execution.activeDeadlineSeconds` | `integer`, default `900` | This resource requires: where `execution` and `execution.activeDeadlineSeconds` are set, execution.connectTimeout must not exceed execution.activeDeadlineSeconds; where `execution` and `execution.activeDeadlineSeconds` are set, policy.lockTimeout must not exceed execution.activeDeadlineSeconds. ActiveDeadlineSeconds is how long one operation Job may run before Kubernetes ends it. An apply that hits this leaves an uncertain outcome, which returns to observation rather than to a replay. |
...
| `spec.policy` | `object`, default `{}` | This resource requires: where `policy` is set, policy.lockTimeout must not exceed execution.activeDeadlineSeconds. Policy decides what may run without a person: the apply mode, the approval requirement, and the locks a run takes. |
...
| `spec.policy.lockTimeout` | `string`, default `5m` | This resource requires: where `policy` and `policy.lockTimeout` are set, policy.lockTimeout must not exceed execution.activeDeadlineSeconds. LockTimeout bounds the wait for the database's own migration lock. It is the lock the engine takes, not the Kubernetes Lease: two controllers that never run at the same time still need the database to serialize them. |
...
| `spec.target.urlFrom.optional` | `boolean` | This resource requires: where `target.urlFrom.optional` is set, target.urlFrom must name a required Secret key. Specify whether the Secret or its key must be defined |
