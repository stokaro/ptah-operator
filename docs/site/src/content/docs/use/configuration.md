...
| `monitoring.prometheusRule.planStoreBytesAbove` | `null` | Bytes of retained schema plans that are too many. The operator prunes no plan, so this is the budget the installation set for them in etcd. |
