...
func TestNewTeardownRBACConvergenceBarrierDiscoversEveryDirectEndpoint(t *testing.T) {
	...
	if got := authorizationSweepSize(barrier); got != 74 {
		t.Fatalf("authorization sweep size = %d, want 74 exact retired-subject plus current-credential probes", got)
	}
...
func TestTeardownAuthorizationSubjectsAndChecksCoverRetiredPrivileges(t *testing.T) {
	...
	if want := map[string]int{"controller": 23, "certificate": 8, "hook-quiesce": 20}; !reflect.DeepEqual(probeCounts, want) {
	...
	if len(checks) != 70 || len(byName) != 70 {
		t.Fatalf("authorization checks = %d total/%d unique, want 70/70", len(checks), len(byName))
	}
...
func TestTeardownAuthorizationChecksUseSeparateCoordinationNamespace(t *testing.T) {
	...
	if len(checks) != 72 {
		t.Fatalf("split-namespace authorization check count = %d, want 72", len(checks))
	}
		...
		for _, controllerServiceAccountCreated := range []bool{false, true} {
			for _, splitCoordinationNamespace := range []bool{false, true} {
				...
				t.Run(name, func(t *testing.T) {
					...
					// the finalizer update, and the two status verbs.
					wantCounts := map[string]int{"controller": 23, "hook-quiesce": 20}
					if certificateEnabled {
