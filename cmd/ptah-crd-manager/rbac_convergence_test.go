...
func TestNewTeardownRBACConvergenceBarrierDiscoversEveryDirectEndpoint(t *testing.T) {
	...
	if got := authorizationSweepSize(barrier); got != 79 {
		t.Fatalf("authorization sweep size = %d, want 79 exact retired-subject plus current-credential probes", got)
	}
...
func TestTeardownAuthorizationSubjectsAndChecksCoverRetiredPrivileges(t *testing.T) {
	...
	if want := map[string]int{"controller": 28, "certificate": 8, "hook-quiesce": 20}; !reflect.DeepEqual(probeCounts, want) {
	...
	if len(selfChecks) != 23 {
		t.Fatalf("current cleanup credential check count = %d, want 23", len(selfChecks))
	}
	...
	if len(checks) != 75 || len(byName) != 75 {
		t.Fatalf("authorization checks = %d total/%d unique, want 75/75", len(checks), len(byName))
	}
...
func TestTeardownAuthorizationChecksUseSeparateCoordinationNamespace(t *testing.T) {
	...
	if len(checks) != 77 {
		t.Fatalf("split-namespace authorization check count = %d, want 77", len(checks))
	}
		...
		for _, controllerServiceAccountCreated := range []bool{false, true} {
			for _, splitCoordinationNamespace := range []bool{false, true} {
				...
				t.Run(name, func(t *testing.T) {
					...
					// The migration resource adds four controller grants: patch,
					// the finalizer update, and the two status verbs. Its plan
					// adds three more: create and the two status verbs.
					wantCounts := map[string]int{"controller": 28, "hook-quiesce": 20}
					if certificateEnabled {
					...
					// 35 rose to 38 when the versioned-migration kinds joined the
					// CRD set: the hook's privileged CRD update is probed per CRD
					// name, and there are three more names. 38 rose to 42 when the
					// controller gained the four PtahMigration grants.
					wantUnion := 47 + wantSelfChecks
					if certificateEnabled {
						wantUnion += 5
					}
...
func validRBACRolloutGuard() *crdupgrade.RolloutGuard {
	return &crdupgrade.RolloutGuard{
		...
		ReleaseSequence:              1,
		ManagerImage:                 "registry.example.test/ptah-operator@sha256:1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef",
