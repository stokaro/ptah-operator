...
# nothing did.
PHASE_REASON_MARKER=${TMPDIR:-/tmp}/ptah-e2e-reason-assert.$$
...
k get validatingwebhookconfiguration/ptah-operator-admission -o json |
	jq -e --arg namespace "$OPERATOR_NAMESPACE" --arg service "$WEBHOOK_SERVICE" \
      ...
      def operation_labels($object):
        ...
        $object + ".metadata.labels[" + $quote + "app.kubernetes.io/component" + $quote + "] in [" + $quote + "schema-operation" + $quote + ", " + $quote + "migration-operation" + $quote + "])";
      def operation_owner($object):
        ...
        $quote + "^ptah-(m-)?(resolve|verify|observe|plan|history|apply)-" + $quote + ")))";
...
cleanup_files() {
	...
	if [ -n "${PHASE_REASON_MARKER:-}" ]; then
		if [ "$status" -ne 0 ] && [ ! -f "$PHASE_REASON_MARKER" ]; then
			printf 'e2e assertions: exited with status %s at a command that failed under set -e; no proof reported a reason\n' "$status" >&2
		fi
