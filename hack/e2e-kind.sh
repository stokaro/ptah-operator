...
# The controller-state version every phase asserts against is the one the chart
# it deploys was stamped with, so it is read out of the Makefile that stamps it
# rather than written here again. A second copy of the number agrees with the
# chart only until the contract moves, and the disagreement then surfaces as an
# admission guard refusing a Job an hour into a lifecycle.
