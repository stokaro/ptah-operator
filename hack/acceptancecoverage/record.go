...
func (c *coverage) recordMarkdown(root string) string {
	...
	out.WriteString("No requirement is marked passed here. A closed issue, a merged fix and a green run\n")
	out.WriteString("against a pull request are none of them evidence about a candidate.\n\n")

	out.WriteString("## Candidate\n\n| Field | Value |\n| --- | --- |\n")
	for _, field := range candidateFields(root, c) {
		value := field.value
		if value == "" {
	...
	for _, entry := range requirements {
		fmt.Fprintf(&out, "| **%s** %s | Not assessed | %s | %s |\n",
			entry.id, entry.title, entry.needs, entry.repository)
	}
...
// cannot.
func candidateFields(root string, c *coverage) []candidateField {
	return []candidateField{
		...
		{name: "Manager image digest", supplied: "the digest the release publishes"},
		{name: "Runner image digest", supplied: "the digest the release publishes"},
		{name: "Executor image digest", supplied: "the digest the release publishes"},
		{name: "Chart digest", supplied: "the digest of the packaged chart"},
		{name: "Effective installation values", supplied: "the values the qualified profile installs with"},
		{name: "Operating targets", supplied: "the latency, throughput and capacity limits the profile claims"},
		{name: "Recovery objectives", supplied: "the recovery point and time objectives, recorded separately for the database and for operator state"},
		{name: "Run evidence", supplied: "the CI run and job identities the executed scenarios come from"},
	}
}
